...
REM =================================================================================
REM 이 스크립트는 CATANet 모델의 데이터셋 준비, 프루닝, 파인튜닝, 재구성,
REM 분석까지의 전체 워크플로우를 자동으로 실행합니다.
REM
REM 실행 방법:
...
REM --- 1. 데이터셋 다운로드 및 압축 해제 ---
echo.
echo [STEP 1/7] DIV2K 데이터셋 존재 여부를 확인합니다...
if not exist datasets\DIV2K (
    echo [INFO] 'datasets\DIV2K' 폴더가 없습니다. 다운로드를 시작합니다.
    if not exist datasets mkdir datasets
    cd datasets
    echo [INFO] DIV2K.tar 다운로드 중... (용량이 크므로 시간이 오래 걸릴 수 있습니다)
    powershell -Command "Invoke-WebRequest -Uri 'https://cv.snu.ac.kr/research/EDSR/DIV2K.tar' -OutFile 'DIV2K.tar'"
    if %errorlevel% neq 0 (
        echo [ERROR] 데이터셋 다운로드 중 오류가 발생했습니다.
        cd ..
        exit /b %errorlevel%
    )
    echo [INFO] 다운로드 완료. 압축을 해제합니다...
    tar -xf DIV2K.tar
    if %errorlevel% neq 0 (
        echo [ERROR] 압축 해제 중 오류가 발생했습니다. 'tar' 명령이 설치되어 있는지 확인하세요.
        cd ..
        exit /b %errorlevel%
    )
    del DIV2K.tar
    cd ..
    echo [SUCCESS] 데이터셋 준비 완료.
) else (
    echo [INFO] DIV2K 데이터셋이 이미 존재합니다. 다운로드를 건너뜁니다.
)


REM --- 2. 이전 결과 파일 정리 ---
echo.
echo [STEP 2/7] 이전 실행 결과 파일들을 정리합니다...
...
REM --- 3. 모델 프루닝 ---
echo.
echo [STEP 3/7] 모델 프루닝을 시작합니다. (run_catanet_pruning.py)
...
REM --- 4. 지식 증류 방식별 파인튜닝 ---
echo.
echo [STEP 4/7] 지식 증류 방식별 모델 파인튜닝을 시작합니다. (finetune_pruned_model.py)

echo.
echo [INFO] 4-1. Output Distillation 방식으로 파인튜닝...
...
echo.
echo [INFO] 4-2. Feature Distillation 방식으로 파인튜닝...
...
REM --- 5. 압축 모델 재구성 ---
echo.
echo [STEP 5/7] 파인튜닝된 모델들을 물리적으로 작은 모델로 재구성합니다. (analysis/rebuild_pruned_model.py)

echo.
echo [INFO] 5-1. Output KD 모델 재구성...
...
echo.
echo [INFO] 5-2. Feature KD 모델 재구성...
...
REM --- 6. 가중치 희소성 검사 ---
echo.
echo [STEP 6/7] 생성된 모든 모델의 가중치 희소성을 검사합니다. (analysis/inspect_weights.py)
...
REM --- 7. 최종 성능 비교 및 시각화 ---
echo.
echo [STEP 7/7] 모든 모델의 성능을 최종 비교하고 트레이드오프 그래프를 생성합니다. (analysis/visualize_tradeoff.py)
